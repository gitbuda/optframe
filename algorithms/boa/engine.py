...
class BOASolution(object):
    '''
    bitstr (bit string) and fitness
    '''
    def __init__(self, bitstr, fitness):
        self.bitstr = bitstr
...
def margin_probability(i, population):
    '''
    '''
    prob = 1.0 * sum(map(lambda x: x.bitstr[i], population)) / len(population)
    return prob
...
def sample(graph, population):
    ...
    return BOASolution(bitstr, None)
...
def run(context):
    ...
    try:
        ...
        population = [BOASolution(x, evaluator.evaluate(x)) for x in
        ...
        for iteration in xrange(iterations_number):
            ...
            for children in childrens:
                children.fitness = evaluator.evaluate(children.bitstr)
            ...
            first = population[0]
            print 'Best solution: %s Fitness %s' % (first.bitstr,
                                                    first.fitness)
    ...
    return best.bitstr, best.fitness
